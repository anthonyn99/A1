...
async function handleNotifDebug(request, env, origin) {
  ...
  if (url.searchParams.get('send') === '1') {
    ...
    for (const d of targets) {
      ...
      try {
        await sendFCM(projectId, token, 'WORKER TEST ✓ ' + new Date().toLocaleTimeString(), 'workertest_' + Date.now(), accessToken);
...
async function runReminders(env) {
  ...
  await Promise.allSettled(due.map(async (r) => {
    ...
    targets.forEach(token => jobs.push(
      sendFCM(projectId, token, title, id, accessToken)
...
async function sendFCM(projectId, token, title, id, accessToken) {
  const res = await fetch(`https://fcm.googleapis.com/v1/projects/${projectId}/messages:send`, {
    ...
    body: JSON.stringify({
      message: {
        ...
        data: { id: String(id || ''), title: String(title || 'Task reminder'), body: String(title || 'Task reminder') },
