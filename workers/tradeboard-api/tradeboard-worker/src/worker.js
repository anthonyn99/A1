...
export default {
  ...
  // Cron: 6:30 AM Mountain (12:30 UTC summer/MDT) Mon-Fri.
  // Pulls latest filled orders from Webull + patches any missing fees into KV.
  // Runs silently whether the app is open or not — Journal is ready on first open.
  async scheduled(event, env, ctx) {
    ctx.waitUntil(
      syncJournalFromWebull(env)
        .then(trades => console.log('[cron] journal sync done, trades:', trades?.length ?? 0))
        .catch(e  => console.error('[cron] journal sync failed:', e.message))
    );
  },
};
