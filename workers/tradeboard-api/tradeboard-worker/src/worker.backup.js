...
async function fetchBalance(accountId, env) {
  ...
  return {
    ...
    /* buying_power was reading only the per-currency object; Webull places the
       usable figure under varying keys. Try the common variants in priority order. */
    buyingPower:       pick('buying_power','available_buying_power','day_buying_power','total_buying_power'),
...
async function syncJournalFromWebull(env) {
  try {
    ...
    /* Strip incorrectly-calculated fees from all entries (fees were auto-calculated in a prior deploy) */
    let feesStripped = 0;
    journal.trades.forEach(t => {
      (t.legs || []).forEach(l => {
        if (l.fee && parseFloat(l.fee) > 0 && l.fee !== '0') {
          l.fee = 0;
          t.commission = 0;
          feesStripped++;
        }
    ...
    if (added > 0 || feesStripped > 0) {
      await saveJournal(env, journal);
      console.log(`[journal] Synced ${added} new trade(s), stripped fees from ${feesStripped} leg(s)`);
    }
