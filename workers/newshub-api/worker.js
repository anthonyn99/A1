...
const BATCH_SIZE = 6;        // 6 events/batch — small enough that the model
                             // reliably returns a verdict for EVERY event in the
                             // batch (at 8-10 it silently omitted ~half, leaving
                             // them raw and tripping the degraded banner).
const MAX_EVENTS = 36;       // top 36 by importance = 6 batches of 6, run as one
                             // parallel wave (concurrency 10) so the AI phase still
                             // finishes well inside the time budget.
...
const AI_SUBREQUEST_BUDGET = 24; // first pass (~6) + omission-retry passes. With
                                 // Finnhub-only fetch (~28 subreq) this stays under
                                 // the 50/invocation cap.
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors, prefetchedArticles){
  ...
  if (batches.length && avail.length){
    ...
    // re-run on a fresh model. This is the main lever against "majority RAW".
    for (let round = 0; round < 2 && aiBudgetLeft() && aiCallBudgetLeft(); round++){
      const haveVerdict = new Set();
      for (const bi of Object.keys(outputs)){
        const out = outputs[bi]; if (!out) continue;
        for (const o of out) haveVerdict.add(o.id);
      }
      ...
      const model = avail[(round+2) % avail.length];
      const retried = await mapLimit(retryBatches, AI_CONCURRENCY,
        b => callModel(model, b, env, ctx, wl, sectors).catch(()=>null));
      // Append each retry batch to `batches` and store its output at the SAME
      // index, so the merge loop (which reads outputs[bi] for bi over batches)
      // picks them up correctly.
      retryBatches.forEach((b, k) => {
        const idx = batches.length; // next free index
        batches.push(b);
  ...
  for (let bi=0; bi<batches.length; bi++){
    ...
    for (const ev of batches[bi]){
      ...
      if (!ai.primaryTicker || ai.primaryTicker === 'NONE'){
        aiRejectedIds.add(ev.id); // AI judged it off-watchlist → drop
        continue;
      }
