...
const BATCH_SIZE = 12;       // small batches = fast individual AI calls
const MAX_EVENTS = 60;       // top 60 events by importance reach AI (5 batches)
...
const AI_CALL_TIMEOUT = 22000; // ms — abandon a single AI call that hangs so it
                             // can't stall the whole (background) build
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors){
  ...
  if (working && batches.length > 1){
    const rest = await Promise.all(
      batches.slice(1).map(b => callModel(working, b, env, ctx, wl, sectors).catch(() => null))
    );
