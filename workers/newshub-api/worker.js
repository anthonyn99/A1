...
const AI_CHAIN = [
  { provider:'gemini', model:'gemini-2.5-flash-lite' },   // PRIMARY: fastest, highest RPD, free tier
  { provider:'gemini', model:'gemini-2.0-flash' },        // fallback 1
  { provider:'nim',    model:'mistralai/mixtral-8x7b-instruct' }, // fallback 2: much faster than 70B
  { provider:'gemini', model:'gemini-2.5-flash' },        // fallback 3
  { provider:'gemini', model:'gemini-1.5-flash' },        // fallback 4
  { provider:'nim',    model:'meta/llama-3.3-70b-instruct' }, // LAST: confirmed 107s/batch — too slow
];
...
const AI_CONCURRENCY = 2;    // max parallel AI calls. Lowered from 4: bursting
                             // many batches at once tripped Gemini free-tier RPM
                             // (per-minute) limits → 429s → raw. 2 keeps us under
                             // the RPM cap while still finishing well within budget.
...
async function callGemini(model, prompt, env, ctx, blockKey){
  // RPM (per-minute) 429s are transient on the free tier — firing several batches
  // in parallel trips them easily. Those must NOT hour-block the model (that used
  // to cascade the whole build to RAW). Only a DAILY quota 429 earns the long
  // block. We retry RPM 429s a couple times with short backoff.
  ...
  for (let attempt = 0; attempt < 3; attempt++){
    ...
    // Daily-quota signals → long block + bail. Otherwise treat as RPM: brief wait, retry.
    ...
    console.warn(`Gemini ${model} 429 RPM (attempt ${attempt+1}/3) — backing off`);
    await new Promise(res => setTimeout(res, 1200 * (attempt + 1))); // 1.2s, 2.4s
  }
  if (r.status === 429){ // still rate-limited after retries — give up on this batch (no hour block)
    console.warn(`Gemini ${model} 429 RPM persisted after retries — dropping batch`);
    return null;
  }
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors){
  ...
  // ── PROBE: walk the chain to find a working model ───────────────────────
  // Then run every other batch in PARALLEL against that same model. This bounds
  // total wall-time to ~2 AI calls instead of summing all batches sequentially.
  //
  // Hardened: the probe no longer abandons a model after a SINGLE batch[0]
  // failure (a lone truncated/timed-out batch used to nuke the entire build to
  // RAW). It tries each candidate model on up to 2 different batches before
  // moving on, so one flaky batch can't disqualify an otherwise-healthy model.
  const outputs = new Array(batches.length).fill(null);
  let working = null;
  if (batches.length){
    // Probe order: try batch[0], and if that one fails, try batch[1] (if any)
    // before writing the model off entirely.
    const probeIdx = batches.length > 1 ? [0, 1] : [0];
    for (const entry of AI_CHAIN){
      const blockKey = entry.provider === 'nim'
        ? 'quota_block:nim:' + entry.model.replace('/','_')
        : 'quota_block:' + entry.model;
      try { if (await env.NEWSHUB_CACHE.get(blockKey)) continue; } catch(e){}
      for (const pi of probeIdx){
        const out = await callModel(entry, batches[pi], env, ctx, wl, sectors);
        if (out){ working = entry; outputs[pi] = out; console.log(`AI probe locked on ${entry.provider}/${entry.model} (batch ${pi})`); break; }
      }
      if (working) break;
    }
  }
  if (working){
    // Run every not-yet-filled batch against the working model in parallel.
    const todo = batches.map((b, i) => ({ b, i })).filter(x => !outputs[x.i]);
    if (todo.length){
      const rest = await mapLimit(
        todo, AI_CONCURRENCY,
        x => callModel(working, x.b, env, ctx, wl, sectors).catch(() => null)
      );
      rest.forEach((out, k) => { outputs[todo[k].i] = out; });
    }
    // ── Salvage pass: any batches the working model dropped (transient/truncation)
    // get one retry on the NEXT available chain model. Keeps a few flaky batches
    // from silently vanishing while the rest analyzed fine.
    const dropped = batches.map((b, i) => ({ b, i })).filter(x => !outputs[x.i]);
    if (dropped.length){
      for (const entry of AI_CHAIN){
        if (entry === working) continue;
        const bk = entry.provider === 'nim'
          ? 'quota_block:nim:' + entry.model.replace('/','_')
          : 'quota_block:' + entry.model;
        try { if (await env.NEWSHUB_CACHE.get(bk)) continue; } catch(e){}
        const retried = await mapLimit(
          dropped, AI_CONCURRENCY,
          x => callModel(entry, x.b, env, ctx, wl, sectors).catch(() => null)
        );
        let any = false;
        retried.forEach((out, k) => { if (out){ outputs[dropped[k].i] = out; any = true; } });
        if (any) break; // one salvage model is enough
      }
    }
  }
