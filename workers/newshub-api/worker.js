...
// The default WATCHLIST/SECTORS above are used by the cron pre-warm and as a
// fallback. On-demand /news requests may pass ?tickers=A,B,C — in that case the
// whole pipeline runs against the caller's list, and sectors are inferred from
// this table (mirrors the client TB_TICKER_SECTORS map). Unknown tickers fall
// back to 'Diversified' so news still surfaces for them.
const SECTOR_LOOKUP = {
...
// Sanitize an incoming ?tickers= list → uppercase, A-Z./- only, deduped, capped.
...
async function geminiBatch(events, env, ctx, wl, sectors){
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors){
  ...
  for (let i=0; i<events.length; i+=BATCH_SIZE){
    ...
    let out = [];
    try {
      out = await geminiBatch(batch, env, ctx, wl, sectors);
...
async function buildAndCache(env, ctx, useLimitedAPIs, wl, sectors, cacheKey, lockKey){
  ...
  if (!result.degraded){
    await env.NEWSHUB_CACHE.put(cacheKey, body, { expirationTtl: CACHE_TTL });
  }
  await env.NEWSHUB_CACHE.delete(lockKey); // release build lock
  console.log(`Pipeline done: ${result.events.length} events, degraded=${result.degraded}, wl=${wl.length}`);
  return body;
}
...
export default {
  ...
  async fetch(req, env, ctx){
    ...
    // list. Falls back to the default WATCHLIST (shared with cron) when absent.
    // Cache + lock are namespaced per-list so different watchlists never collide.
    const customTickers = parseTickers(url.searchParams.get('tickers'));
    const wl       = customTickers || WATCHLIST;
    const sectors  = customTickers ? sectorsFor(wl) : SECTORS;
    const isCustom = !!customTickers;
    ...
    await env.NEWSHUB_CACHE.put(lockKey, '1', { expirationTtl: 120 }); // auto-expires after 2min if build crashes
