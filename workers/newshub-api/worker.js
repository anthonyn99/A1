...
async function handleStage(env, ctx, req, buildId, stage, sliceIdx){
  ...
  if (stage === 'fetch'){
    ...
    const next = i + 1;
    // Dispatch the next stage and AWAIT it. With the SELF service binding each
    // call is a genuinely separate invocation (own 50-subrequest budget), so this
    // is safe — we're not accumulating budget, just chaining. Awaiting (vs
    // waitUntil) guarantees the dispatch actually fires before this invocation
    // ends; waitUntil was getting cut short, stalling the chain after slice 0.
    if (next < slices.length){
      await kickStage(env, origin, buildId, 'fetch', next);
    } else {
      await kickStage(env, origin, buildId, 'ai', null);
    }
