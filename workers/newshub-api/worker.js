      ...
      try {
        ...
        for (let bi=0; bi<tryBatches.length; bi++){
          ...
          for (const entry of AI_CHAIN){
            ...
            const t0 = Date.now();
            let res;
            try { res = await callModel(entry, tryBatches[bi], env, null, WATCHLIST, SECTORS); }
            catch(e){ res = null; batchTrace.attempts.push({ model: entry.model, threw: e.message, ms: Date.now()-t0 }); continue; }
            batchTrace.attempts.push({ model: entry.model, ok: !!res, count: res?res.length:0, ms: Date.now()-t0 });
            if (res) break; // success — stop walking chain for this batch
          }
