...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors, prefetchedArticles){
  ...
  for (let bi=0; bi<batches.length; bi++){
    ...
    for (const ev of batches[bi]){
      ...
      const candSet = new Set(ev.candidateTickers || []);
      const aiTickerValid = ai.primaryTicker && ai.primaryTicker !== 'NONE'
        && wlSet.has(ai.primaryTicker) && candSet.has(ai.primaryTicker);
      if (!aiTickerValid){
        // Either the AI rejected it (NONE / off-watchlist) OR it hallucinated a
        // ticker the article never mentioned (primaryTicker not in the event's
        // own candidate set — e.g. tagging a Lucid story as CRDO). In the reject
        // case drop it; in the hallucination case fall through to raw with the
        // article's real candidate ticker so it still shows correctly.
        if (ai.primaryTicker === 'NONE' || !ai.primaryTicker){
