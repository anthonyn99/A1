  ...
  async fetch(req, env, ctx){
    ...
    if (url.pathname === '/_stage-debug'){
      ...
      if (url.searchParams.get('trigger') === '1'){
        ...
        await env.NEWSHUB_CACHE.put(lk, '1', { expirationTtl: 600 });
        ctx.waitUntil(startStagedBuild(env, ctx, req, true, tw, sc, ck, lk).catch(async(e)=>{ await env.NEWSHUB_CACHE.delete(lk); }));
        return new Response(JSON.stringify({ triggered:true, cacheKey:ck, tickers:tw.length, note:'wait ~2min then re-check /_stage-debug' }, null, 2),
          { headers:{...cors(),'Content-Type':'application/json'} });
      }
