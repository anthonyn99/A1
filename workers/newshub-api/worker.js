...
const MAX_EVENTS = 56;       // top 56 events by importance (7 batches of 8)
...
const IMPORTANT_KW = /(upgrade|downgrade|price target|raises|cuts guidance|guidance|earnings|beats|misses|acquir|merger|lawsuit|\bsec\b|investigation|recall|partnership|contract|launch|approval|\bfda\b|deal|buyback|dividend|surge|plunge|soar|tumble|record high|all-time|breakout|guides)/i;
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors){
  ...
  // ── PROBE: walk the chain ONCE on batch[0] to find a working model ───────
  ...
  // (the old design summed ~5+ slow NIM calls and blew the Worker time budget,
  // so the build never cached → only stale RAW survived).
  ...
  if (batches.length){
    for (const entry of AI_CHAIN){
      ...
      const out = await callModel(entry, batches[0], env, ctx, wl, sectors);
      if (out){ working = entry; outputs[0] = out; console.log(`AI probe locked on ${entry.provider}/${entry.model}`); break; }
    }
  }
  if (working && batches.length > 1){
    const rest = await mapLimit(
      batches.slice(1), AI_CONCURRENCY,
      b => callModel(working, b, env, ctx, wl, sectors).catch(() => null)
    );
    rest.forEach((out, i) => { outputs[i+1] = out; });
  }
