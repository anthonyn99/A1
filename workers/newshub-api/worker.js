...
const AI_CHAIN = [
  ...
  { provider:'nim',    model:'meta/llama-3.3-70b-instruct', slow:true }, // LAST: ~107s/batch — salvage only
];
...
// ─── Tiingo news (free tier). Multi-symbol in one call. Requires TIINGO_KEY. ──
async function fetchTiingo(symbols, env, isoFrom){
  if (!env.TIINGO_KEY) return [];
  try {
...
async function fetchAllSources(env, useLimitedAPIs, wl){
  ...
  if (useLimitedAPIs){
    ...
    if (tgOk && env.TIINGO_KEY){ tasks.push(fetchTiingo(wl, env, isoFrom).then(r=>{tg=r;return bumpBudget(env,'tiingo');})); }
...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors){
  ...
  if (batches.length && avail.length){
    ...
    for (let round = 1; round <= 2 && dropped.length && avail.length > 1; round++){
...
export default {
  ...
  async fetch(req, env, ctx){
    ...
    // /tiingo-test — raw Tiingo probe so we can see WHY tg=0 (403? empty? format?)
    if (url.pathname === '/tiingo-test'){
      if (!env.TIINGO_KEY) return new Response(JSON.stringify({error:'TIINGO_KEY not set'}), {status:500, headers:{...cors(),'Content-Type':'application/json'}});
      try {
        const start = new Date(Date.now()-72*3600*1000).toISOString().slice(0,10);
        const u = `https://api.tiingo.com/tiingo/news?tickers=aapl,nvda,intc&startDate=${start}&limit=5`;
        const r = await fetch(u, { headers:{ 'Authorization':'Token '+env.TIINGO_KEY, 'Content-Type':'application/json' } });
