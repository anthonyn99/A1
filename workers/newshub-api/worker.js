...
async function fetchEarningsCalendar(wl, env, fromD, toD){
  ...
  try {
    ...
    if (!r.ok){ console.warn('Finnhub earnings cal', r.status); return []; }
    const j = await r.json();
...
async function fetchMacroCalendar(env, fromD, toD){
  const prompt =
...
Use the real published schedule. If unsure of an exact date, omit that item. Dates must be within ${fromD}..${toD}.`;
  ...
  for (const model of CAL_MACRO_MODELS){
    ...
    try {
      ...
      if (!r.ok){ console.warn(`macro ${model} HTTP ${r.status}`); continue; }
      const j = await r.json();
    ...
    } catch(e){ console.warn(`macro ${model} err`, e.message); }
  }
  return [];
}
...
async function buildCalendar(wl, env, days){
  const fromD = etDateStr(0);
  const toD   = etDateStr(Math.min(days, CAL_DAYS_MAX));
  const [earn, macro] = await Promise.all([
    fetchEarningsCalendar(wl, env, fromD, toD),
    fetchMacroCalendar(env, fromD, toD),
  ]);
