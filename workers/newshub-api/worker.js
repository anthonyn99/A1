...
async function runPipeline(env, ctx, useLimitedAPIs, wl, sectors, prefetchedArticles){
  ...
  // Raw fallback applies ONLY to events the AI never processed (a batch that
  // failed entirely). Events the AI saw and REJECTED (NONE) are intentionally
  // dropped above — they're off-watchlist noise, not analysis failures.
  const enrichedIds = new Set(enriched.map(e => e.id));
  let rawCount = 0;
  for (let bi=0; bi<batches.length; bi++){
    if (aiProcessedBatch.has(bi)) continue; // AI handled this batch (enriched or rejected)
    for (const ev of batches[bi]){
