...
export default {
  ...
  async scheduled(event, env, ctx){
    ...
    // ── Catalysts pre-warm — Sunday + Wednesday 6:30 MT ("30 12 * * 0,3") ──
    // Builds the macro+earnings calendar into its KV cache so the Catalysts tab
    // is ready the moment the app opens on those days (cross-device via KV).
    if (cron === '30 12 * * 0,3'){
      ctx.waitUntil(prewarmCalendar(env).catch(e => console.error('Cron calendar failed:', e.message)));
      return;
    }

    // ── News pre-warm — Mon-Fri 6:30 MT ("30 12 * * 1-5") ──
    // Single daily run, so do the RICH build (all sources) for best quality.
    // Per-hour dedup guards against a double tick building twice. Builds for the
    // live TB_WL so the warmed cache matches what the app requests.
