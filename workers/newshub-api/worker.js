...
async function startStagedBuildCron(env, ctx, origin, useLimitedAPIs){
  ...
  await env.NEWSHUB_CACHE.put(lockKey, '1', { expirationTtl: 180 });
...
async function buildAndCache(env, ctx, useLimitedAPIs, wl, sectors, cacheKey, lockKey, prefetchedArticles){
  ...
  const result = await runPipeline(env, ctx, useLimitedAPIs, wl, sectors, prefetchedArticles);
  const body = JSON.stringify({
...
const STAGE_TTL = 360; // staging data lives 6 min — exceeds the 300s build lock so it can't expire mid-build
...
async function kickStage(env, origin, buildId, stage, sliceIdx){
  ...
  try {
    if (env.SELF && typeof env.SELF.fetch === 'function'){
      // Hostname is arbitrary for a service binding; the path is what routes.
      await env.SELF.fetch('https://self' + path, { method:'POST' });
      return;
    }
  } catch(e){ console.error('kickStage SELF dispatch failed:', e.message); }
  // Fallback: HTTP self-fetch (works where loopback is allowed).
  const u = (origin || env.WORKER_ORIGIN || '') + path;
  await fetch(u, { method:'POST' }).catch(e => console.error('kickStage HTTP dispatch failed:', e.message));
}
...
async function handleStage(env, ctx, req, buildId, stage, sliceIdx){
  ...
  if (stage === 'ai'){
    ...
    try {
      // Final stage: cluster + AI + cache, using the staged articles (no re-fetch).
      await buildAndCache(env, ctx, useLimited, wl, sectors, meta.cacheKey, meta.lockKey, articles);
    } catch(e){
      // A throw here (e.g. AI parse/cluster bug) would otherwise leave NO cache →
      // the client polls forever ("Building…"). Capture the error so it's visible
      // via /_stage-debug, then write a raw-degraded result so the poller ends and
      // headlines still show.
      const errInfo = { stage:'ai', buildId, msg:e.message, stack:(e.stack||'').slice(0,800), at:Date.now(), articles:articles.length };
      await env.NEWSHUB_CACHE.put('stage:lasterror', JSON.stringify(errInfo), { expirationTtl: 1800 }).catch(()=>{});
      console.error('AI stage threw:', e.message, e.stack);
      try {
        const rawEvents = rawFallbackEvents(articles, wl);
        const body = JSON.stringify({ events: rawEvents, generatedAt: Date.now(), watchlist: wl, sectors, modelsUsed: [], degraded: true });
        await env.NEWSHUB_CACHE.put(meta.cacheKey, body, { expirationTtl: 600 });
      } catch(e2){ console.error('raw fallback also failed:', e2.message); }
    ...
    return new Response(JSON.stringify({ ok:true, stage:'ai', articles:articles.length }),
      { headers:{...cors(),'Content-Type':'application/json'} });
  }
...
export default {
  ...
  async scheduled(event, env, ctx){
    ...
    // Cron builds the full watchlist, which also exceeds the 50-subrequest cap,
    // so it runs the staged build too. Cron has no inbound request, so we use the
    // configured WORKER_ORIGIN (set as a Worker var). If unset, fall back to an
    // inline build (correct, but capped — only safe for a trimmed watchlist).
    const origin = env.WORKER_ORIGIN || null;
    if (origin){
      ctx.waitUntil(
        startStagedBuildCron(env, ctx, origin, useLimited)
          .catch(e => console.error('Cron staged build failed:', e.message))
      );
    } else {
  ...
  async fetch(req, env, ctx){
    ...
    const lockTtl = (env.STAGE_SECRET && !fitsOneInvocationEarly) ? 300 : 180;
    ...
    const fitsOneInvocation = wl.length <= TICKERS_PER_SLICE; // ~12 tickers → ≤40 subreq
    const canStage = !!env.STAGE_SECRET && (!!env.SELF || !!selfOrigin(req) || !!env.WORKER_ORIGIN);
