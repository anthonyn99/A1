...
const AI_PHASE_BUDGET_MS = 20000; // 20s hard cap on the AI phase. The whole build
  ...
  async fetch(req, env, ctx){
    ...
    if (url.pathname === '/_stage-debug'){
      ...
      if (url.searchParams.get('trigger') === '1'){
        ...
        // Inline build, useLimited=true, NO rate-limit check (test path).
        ctx.waitUntil(
          buildAndCache(env, ctx, true, tw, sc, ck, lk)
            .catch(async(e)=>{ console.error('trigger build failed:', e.message); await env.NEWSHUB_CACHE.delete(lk); })
        );
        return new Response(JSON.stringify({ triggered:true, mode:'inline', cacheKey:ck, tickers:tw.length, note:'wait ~30s then check /news?cacheOnly=1' + (isCustomT?('&tickers='+tw.join(',')):'') }, null, 2),
          { headers:{...cors(),'Content-Type':'application/json'} });
      }
