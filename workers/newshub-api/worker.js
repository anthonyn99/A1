...
// 12 tickers × 3 = 36 subrequests, leaving headroom for limited sources (~4) +
// general (1) + KV ops on the first slice.
const TICKERS_PER_SLICE = 12;
...
async function startStagedBuild(env, ctx, req, useLimitedAPIs, wl, sectors, cacheKey, lockKey){
  ...
  // Fire the first fetch stage (self-invocation). If self-origin is unavailable
  // (e.g. cron with no inbound req), fall back to an inline single-invocation
  // build — correct, just subject to the cap (fine for cron's lighter load).
...
async function kickStage(env, origin, buildId, stage, sliceIdx){
  const u = `${origin}/_stage?build=${encodeURIComponent(buildId)}&stage=${stage}` +
            (sliceIdx!=null ? `&slice=${sliceIdx}` : '') +
            `&key=${encodeURIComponent(env.STAGE_SECRET||'')}`;
  // Brief timeout: we only need the request to LAND and start a new invocation.
  await fetch(u, { method:'POST' }).catch(e => console.error('kickStage dispatch failed:', e.message));
}
...
async function handleStage(env, ctx, req, buildId, stage, sliceIdx){
  ...
  if (stage === 'fetch'){
    ...
    if (next < slices.length){
      ctx.waitUntil(kickStage(env, origin, buildId, 'fetch', next));
    } else {
      ctx.waitUntil(kickStage(env, origin, buildId, 'ai', null));
    }
...
export default {
  ...
  async fetch(req, env, ctx){
    ...
    const canStage = !!env.STAGE_SECRET && !!selfOrigin(req);
