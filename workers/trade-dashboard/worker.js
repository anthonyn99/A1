...
async function runStageA(facts, env){
  ...
  for(const model of GROUNDED_MODELS){
    // skip a model that 429'd recently (grounding quota) — cools down 1h
    ...
    for(let retry=0; retry<=2; retry++){
      try{
        ...
        const transient = e.code==='SERVER' || e.name==='TimeoutError' || e.code==='EMPTY';   // 503/timeout/empty → retry same model
        if(transient && retry<2){ await sleep(1200*(retry+1)+Math.random()*500); continue; }
        break;  // hard error → next model
      }
    }
  }
  // all grounded models failed → no narrative; Stage B still renders hard data.
