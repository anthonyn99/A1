...
const AI_TIMEOUT_MS   = 16 * 1000;    // per-model fetch timeout — fast or dead
const VERSION = '2026-06-22-parse-salvage';
...
const CHUNK_WALK_BUDGET_MS = 25 * 1000; // hard cap per chunk walk — degrade before CF kills it
...
const AI_CHAIN = [
  { provider: 'gemini', models: [
      'gemini-2.5-flash',      // fast + reliable on free tier → lead with it
      'gemini-2.0-flash',
      'gemini-2.0-flash-lite',
      'gemini-2.5-pro',        // slow + tight quota → last resort, not first
  ]},
...
async function callGemini(model, messages, env, maxTokens) {
  ...
  const body = {
    ...
    generationConfig: { temperature: 0.4, maxOutputTokens: maxTokens || MAX_OUT_TOKENS, responseMimeType: 'application/json' },
  };
