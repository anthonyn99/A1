...
const GDELT_BUCKETS = [
  { key:'geopolitics', label:'Geopolitics / Middle East',
    q:'(Iran OR "Strait of Hormuz" OR Israel OR "Middle East" OR Houthi OR Ukraine OR Taiwan) (oil OR market OR military OR strike OR sanction OR war)' },
  { key:'tariffs',     label:'Trade / Tariffs / Trump',
    q:'(tariff OR "trade war" OR Trump OR "executive order") (market OR stocks OR economy OR China OR import)' },
  { key:'fed',         label:'Fed / Rates / Inflation',
    q:'("Federal Reserve" OR Powell OR "interest rate" OR "rate cut" OR inflation OR FOMC) (market OR economy OR yields)' },
  { key:'oil',         label:'Oil / OPEC / Energy',
    q:'(OPEC OR "crude oil" OR "oil prices" OR "oil supply" OR "energy market")' },
];
...
async function gdeltBucket(bucket){
  ...
  const url=`${base}?query=${encodeURIComponent(bucket.q)}&mode=artlist&maxrecords=20&sort=datedesc&timespan=24h&format=json`;
  try{
    const r=await fetch(url,{signal:AbortSignal.timeout(7000),headers:{'User-Agent':'MacroBoard/1.0'}});
    if(!r.ok) return { key:bucket.key, label:bucket.label, count:0, articles:[] };
    const j=await r.json();
    ...
    return { key:bucket.key, label:bucket.label, count:uniq.length, articles:uniq.slice(0,10) };
  }catch{ return { key:bucket.key, label:bucket.label, count:0, articles:[] }; }
}
