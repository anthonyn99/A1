...
const VERSION = '2026-06-21-inline-build';
...
/* BUILD STAGE: runs in ONE self-invocation (kicked once from /build). Fetches
   live facts, then renders EVERY chunk sequentially right here — no per-chunk
   self-invoke hop (that nested ctx.waitUntil after the response returned wasn't
   firing, so chunks never ran). Each chunk is small (4096 tok) + flash-first, so
   the loop is fast and I/O-bound. Progress is written after each chunk so /poll
   sees the stage advance. */
async function runFactsStage(runId, env, ctx) {
  const job = await kvGet(env, 'td_job');
  if (!job || job.runId !== runId) { console.warn('[build] stale/missing job'); return; }
  ...
  // SINGLE, non-sectioned prompt → one full-report call (bigger token budget).
  if (job.single) {
    try {
  ...
  // 2) MULTI-CHUNK → render every chunk sequentially in THIS invocation.
  const dead = new Set(job.deadModels || []);
  const buildDeadline = Date.now() + BUILD_BUDGET_MS;
  for (let i = job.stage; i < job.total; i++) {
    if (Date.now() > buildDeadline) { console.warn('[build] global budget hit, finalizing partial'); break; }
    const isFirst = i === 0;
    try {
      ...
      // keep going — a failed chunk contributes no blocks, the rest still render
    }
    job.stage = i + 1;
    job.deadModels = [...dead];
    await kvPut(env, 'td_job', job);                    // progress → /poll advances
  }

  // 3) Finalize. Any blocks at all → ready (partial is better than nothing).
  const generatedAt = Date.now();
  if (job.blocks.length === 0) {
    await kvPut(env, 'td_report', { status: 'degraded', report: null,
      ...
      generatedAt, model: null, runId, lastAttempts: job.lastAttempts || null });
  } else {
...
async function runStage(runId, env, ctx) {
  ...
  if (job.stage < job.total) {
    ...
    const generatedAt = Date.now();
    if (job.blocks.length === 0) {
      await kvPut(env, 'td_report', { status: 'degraded', report: null, error: 'All chunks failed to render.', generatedAt, model: null, runId, lastAttempts: job.lastAttempts || null });
    } else {
      await kvPut(env, 'td_report', { status: 'ready', report, error: null, generatedAt, model: job.model, runId });
      await kvPut(env, `td_cache:${job.hash}`, { report, generatedAt, model: job.model }, CACHE_TTL_S);
      await bumpRate(env);
