...
const AI_TIMEOUT_MS   = 24 * 1000;    // per-model fetch timeout (flash answers well under this; a slower model is treated as dead fast)
// Bump on every deploy so GET / and /diag confirm which code is actually live.
const VERSION = '2026-06-22-chunk0-inline';
...
// A chunk is only ~1/4 of the report. Asking for 16k tokens makes generation
// slow enough to blow the 24s abort, which cascades the whole chain. Cap chunks.
const CHUNK_MAX_OUT_TOKENS = 4096;
// Hard wall-clock budget for ONE chunk's whole chain walk. Stops a single
// invocation from grinding past Cloudflare's limit (which kills it silently,
// leaving stage stuck at 0). On hit, the chunk degrades-with-reason instead.
const CHUNK_WALK_BUDGET_MS = 60 * 1000;
// Overall wall-clock cap for the whole build (facts + all chunks) in ONE
// invocation. Stays under the 300s /poll cap; on hit we finalize partial blocks.
