...
const AI_TIMEOUT_MS   = 20 * 1000;    // per-model fetch timeout — headroom for a full thinking-off chunk
const VERSION = '2026-06-22-gemini-thinkoff';
...
const CHUNK_MAX_OUT_TOKENS = 3600;    // enough that a chunk's JSON isn't truncated
const CHUNK_WALK_BUDGET_MS = 28 * 1000; // hard cap per chunk walk — degrade before CF kills it
...
const AI_CHAIN = [
  { provider: 'gemini', models: [
      'gemini-2.5-flash',      // thinking-OFF (see callGemini) → fast + strong → lead
      'gemini-2.5-flash-lite', // thinking-OFF → fastest cheap fallback (replaces dead 2.0-flash*)
      'gemini-2.5-pro',        // slow + tight quota, low think budget → last resort, not first
