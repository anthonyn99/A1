...
const AI_TIMEOUT_MS   = 22 * 1000;    // per-model fetch cap — must leave room under the ~30s isolate wall limit to kick next stage
const VERSION = '2026-06-22-lean-stages';
...
const CHUNK_WALK_BUDGET_MS = 24 * 1000; // walk cap — after one ~22s attempt, stop (don't start a 2nd that blows the isolate)
...
async function fhQuotes(symbols, env) {
  const out = {};
  const batch = 32;   // bigger batch = fewer serial rounds (quotes fire in parallel per batch)
  const run = async list => {
    for (let i = 0; i < list.length; i += batch) {
      ...
      if (i + batch < list.length) await sleep(120);
    }
  };
  await run(symbols);
  // No retry pass: a couple of missed quotes (e.g. transient Yahoo miss) just get
  // omitted from facts. Worth it — the retry added ~8s and the facts stage must
  // stay lean to reliably hand off within the wall-time budget.
  return out;
}
...
async function runBuild(prompt, env, ctx) {
  ...
  // STAGED across invocations — each gets a FRESH ~30s wall-time budget. This is
  // mandatory: ctx.waitUntil does NOT extend an isolate's wall-time limit, so a
  // build that needs minutes of total work MUST split across invocations. Each
  // stage is kept lean (<~25s) so it reliably finishes AND kicks the next.
  await kickStage(env, ctx, runId, 'facts');
}
...
/* PRE-STAGE: fetch live facts in its OWN invocation, then hand off. It does NOT
   render any chunk — facts(~20s) + a render(~12s) in one isolate = ~32s, which
   exceeds the ~30s wall-time limit and kills the isolate before it can kick the
   next stage. Keeping this stage facts-only makes it lean and reliable. */
async function runFactsStage(runId, env, ctx) {
  ...
  job.facts = facts;
  job.factsDone = true;
  job.stage = 0;            // render starts at chunk 0
  await kvPut(env, 'td_job', job);

  // Hand off to the render pipeline — fresh invocation, fresh wall-time budget.
  // Handles both single (one full-report call) and multi-chunk in runStage.
...
/* Render stage. Runs in its OWN invocation (fresh wall-time budget).
   - single prompt → one full-report call here, then finalize.
   - multi-chunk → render chunk `job.stage`, then kick the next stage or finalize.
   Each invocation does AT MOST one AI call (~12s) so it never nears the wall limit. */
async function runStage(runId, env, ctx) {
  const job = await kvGet(env, 'td_job');
  if (!job || job.runId !== runId) { console.warn('[stage] stale/missing job'); return; }

  // SINGLE, non-sectioned prompt → one full-report call in this fresh isolate.
  if (job.single) {
    try {
      const { report, model } = await aiRoute(buildMessages(job.prompt, job.facts || ''), env);
      const generatedAt = Date.now();
  ...
  try {
    // Facts inject into chunk 0 only (buildChunkMessages handles that).
