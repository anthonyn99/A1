...
const AI_TIMEOUT_MS   = 30 * 1000;    // per-model fetch timeout — 30s for dense thinking-off chunks
const VERSION = '2026-06-22-inline-build';
const MAX_RETRIES     = 0;            // no retries inside a chunk — keeps invocation short
const MAX_OUT_TOKENS  = 16384;        // big enough for a full 10-section report (single path)
const CHUNK_MAX_OUT_TOKENS = 4096;    // headroom for dense chunks
const CHUNK_WALK_BUDGET_MS = 38 * 1000; // hard cap per chunk walk
const BUILD_BUDGET_MS = 220 * 1000;
...
const AI_CHAIN = [
  { provider: 'gemini', models: [
      'gemini-2.5-flash-lite', // thinking-OFF → fastest for structured JSON chunks → lead
      'gemini-2.5-flash',      // thinking-OFF → stronger fallback if lite fails
      'gemini-2.5-pro',        // slow + tight quota, low think budget → last resort only
      // 2.0-flash / 2.0-flash-lite REMOVED — Google shut them down 2026-06-01.
  ]},
  { provider: 'nvidia', models: [
      'nvidia/llama-3.3-nemotron-super-49b-v1',
      'meta/llama-3.3-70b-instruct',
      'qwen/qwen2.5-72b-instruct',
      // 'deepseek-ai/deepseek-r1' removed — reasoning model, 30–60s, blows the timeout.
  ]},
  { provider: 'groq', models: [
      'llama-3.3-70b-versatile',
      'qwen-2.5-32b',
      // 'deepseek-r1-distill-llama-70b' removed — reasoning model, too slow.
  ]},
];
...
async function buildMarketFacts(env) {
  ...
  // Economic calendar + headlines + newshub cross-feed — run in PARALLEL with
  // tight timeouts so the facts stage stays fast (it shares an invocation with
  // chunk 0, so a slow serial chain here would blow the wall-time budget).
  const [eco, newsBlock, nhBlock] = await Promise.all([
    fhFetch(`/calendar/economic?from=${iso(today)}&to=${iso(to)}`, env).catch(() => null),
    buildNewsFacts(env).catch(() => ''),
    buildNewsHubFacts(env).catch(() => ''),
  ]);
  let econLines = [];
...
function buildChunkMessages(chunkPrompt, isFirst, facts) {
  // First chunk: full system doc + facts + date context (sets title/summary/meta).
  // Later chunks: blocks-only system doc + date context ONLY — no facts re-injection.
  // Facts are ~2-3KB; injecting into every chunk was doubling input size and
  // blowing the per-model fetch timeout on chunks 2+.
  const sys = isFirst ? RENDER_SCHEMA_DOC : CHUNK_SCHEMA_DOC;
  const factsBlock = isFirst ? (facts || '') : '';
  return [
    { role: 'system', content: sys },
    { role: 'user',   content: factsBlock + dateContext() + chunkPrompt },
  ];
}
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
async function callGemini(model, messages, env, maxTokens) {
  ...
  const body = {
    ...
    generationConfig: {
      temperature: 0.4,
      maxOutputTokens: maxTokens || MAX_OUT_TOKENS,
      responseMimeType: 'application/json',
      // CRITICAL: 2.5 models default thinking ON. Thinking burns seconds (blows
      // the fetch timeout) AND eats maxOutputTokens (→ truncated/empty JSON).
      // budget 0 = thinking OFF → fast, full output budget for actual report.
      // Pro can't go to 0 (min budget) so give it a small one when reached.
      thinkingConfig: { thinkingBudget: geminiThinkingBudget(model) },
    },
  };
...
/* Thinking budget per Gemini model.
   - flash / flash-lite: 0 → thinking fully off (fastest, restores old behavior).
   - pro: can't fully disable (min budget); give it a small cap so it stays under
     the fetch timeout on the rare runs it's reached as last resort. */
function geminiThinkingBudget(model) {
  return /pro/i.test(model) ? 256 : 0;
}

async function callOpenAICompat(baseUrl, key, model, messages, env, extra = {}, maxTokens) {
  if (!key) throw tagged('NO_KEY', 'key not set');
  const res = await fetchTO(`${baseUrl}/chat/completions`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
    body: JSON.stringify({
      model, messages, temperature: 0.4, max_tokens: maxTokens || MAX_OUT_TOKENS,
...
const callNvidia = (model, messages, env, maxTokens) =>
  callOpenAICompat('https://integrate.api.nvidia.com/v1', env.NVIDIA_KEY, model, messages, env, {}, maxTokens);
const callGroq = (model, messages, env, maxTokens) =>
  callOpenAICompat('https://api.groq.com/openai/v1', env.GROQ_KEY, model, messages, env, {}, maxTokens);
...
/* ════════════════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════════════════ */
function dispatch(provider, model, messages, env, maxTokens) {
  if (provider === 'gemini') return callGemini(model, messages, env, maxTokens);
  if (provider === 'nvidia') return callNvidia(model, messages, env, maxTokens);
  if (provider === 'groq')   return callGroq(model, messages, env, maxTokens);
...
function parseReport(txt, blocksOnly) {
  ...
  if (a === -1) throw tagged('PARSE', 'no JSON object found');
  // First try a clean parse of the outermost object.
  if (b > a) {
    try {
      const obj = JSON.parse(s.slice(a, b + 1));
      if (obj && typeof obj === 'object' && Array.isArray(obj.blocks)) return obj;
    } catch (e) { /* fall through to salvage */ }
  }
  // SALVAGE: the response was likely truncated (token cap) → recover whatever
  // complete block objects we can from the "blocks": [ ... ] array.
  const salvaged = salvageBlocks(s);
  if (salvaged.length) {
    const head = s.slice(a, s.indexOf('"blocks"') > -1 ? s.indexOf('"blocks"') : a + 1);
    const title   = (head.match(/"title"\s*:\s*"([^"]*)"/) || [])[1];
    const summary = (head.match(/"summary"\s*:\s*"([^"]*)"/) || [])[1];
    return { title: title || undefined, summary: summary || undefined, blocks: salvaged };
  }
  throw tagged('PARSE', 'unparseable / no salvageable blocks');
}

/* Pull complete top-level {...} objects out of a (possibly truncated) blocks
   array by tracking brace depth and string state. Drops any trailing partial. */
function salvageBlocks(s) {
  const key = s.indexOf('"blocks"');
  if (key === -1) return [];
  let i = s.indexOf('[', key);
  if (i === -1) return [];
  i++;
  ...
  while (i < s.length) {
    ...
    if (depth !== 0) break; // truncated mid-object → stop, drop the partial
    try { out.push(JSON.parse(s.slice(start, i))); } catch (e) { break; }
  }
  return out;
}
    ...
    for (const model of tier.models) {
      for (let retry = 0; retry <= MAX_RETRIES; retry++) {
        try {
          ...
          // Retryable: 429 / 5xx / empty. Back off then retry SAME model.
          // A timeout won't clear in 800ms, so don't retry it — move on.
          const retryable = (e.code === 'RATE_LIMIT' || e.code === 'SERVER' || e.code === 'EMPTY') && e.name !== 'TimeoutError';
...
async function runBuild(prompt, env, ctx) {
  ...
  const job = { runId, hash, prompt, facts: '', factsDone: false, chunks,
                ...
                deadModels: [], startedAt };
  await kvPut(env, 'td_job', job);

  // Run the ENTIRE build (facts + every chunk) in THIS one waitUntil task,
  // sequentially. NO self-invocation hops — those kept silently dropping at a
  // mid stage and stalling the whole build. The work is I/O-bound (awaiting
  // Gemini), so it stays well under the CPU limit; BUILD_BUDGET_MS guards wall time.
  await runBuildPipeline(job, env);
}

/* The whole build, sequential, in a single invocation. Replaces the fragile
   facts→chunk1→chunk2→… self-hop chain. */
async function runBuildPipeline(job, env) {
  const { runId } = job;
  ...
  // SINGLE, non-sectioned prompt → one full-report call.
  if (job.single) {
    try {
      ...
      await kvPut(env, 'td_report', { status: 'degraded', report: null, error: e.message || 'AI failed', generatedAt: Date.now(), model: null, runId, lastAttempts: e.attempts || null });
    }
    return;
  }

  // MULTI-CHUNK → render each chunk in turn, right here. Facts go into chunk 0
  // only (buildChunkMessages handles that). A throttled/dead model is remembered
  // across chunks via `dead` so it's skipped, not re-timed-out every chunk.
  const dead = new Set(job.deadModels || []);
  for (let i = 0; i < job.chunks.length; i++) {
    if (Date.now() > deadline) {
      job.lastErr = `build budget ${BUILD_BUDGET_MS}ms exceeded at chunk ${i}/${job.total}`;
      break; // finalize with whatever blocks we have
    }
    const isFirst = i === 0;
    try {
      const { report, model } = await aiRouteChunk(buildChunkMessages(job.chunks[i], isFirst, facts), env, dead);
      if (isFirst) {
      ...
      if (e.attempts) job.lastAttempts = e.attempts;
      // keep going — a partial report beats a total timeout
    }
    job.deadModels = [...dead];
    job.stage = i + 1;
    await kvPut(env, 'td_job', job);
...
async function kickStage(env, ctx, runId, phase) {
  ...
  // AWAIT the dispatch (don't nest another ctx.waitUntil inside a waitUntil task —
  // that second one often never registers, which silently stalled the build).
  // The target /_stage handler returns 202 immediately and does its own work in
  // its own waitUntil, so awaiting here only waits for the hop to be accepted.
  try {
    if (target) await target.fetch(new Request(url, { method: 'POST', headers, body }));
    else        await fetch(url, { method: 'POST', headers, body });
  } catch (e) { console.error('[kickStage]', e.message); }
}

/* PRE-STAGE: fetch live facts in its OWN invocation, then hand off to the
   per-chunk pipeline (runStage). Each chunk then renders in its own invocation
   with its own wall-time budget — this is what avoids Cloudflare killing the
   build mid-way (doing all chunks in one waitUntil exceeded the ~30s cap). */
async function runFactsStage(runId, env, ctx) {
  ...
  // 1) Live market facts (own invocation budget).
  let facts = '';
  try { facts = await buildMarketFacts(env); } catch (e) { console.error('[facts]', e.message); }
  job.facts = facts;
  job.factsDone = true;
  ...
  // SINGLE, non-sectioned prompt → one full-report call right here.
  if (job.single) {
    try {
      const { report, model } = await aiRoute(buildMessages(job.prompt, facts), env);
      const generatedAt = Date.now();
  ...
  // MULTI-CHUNK → render chunk 0 INLINE here (facts + one chunk fits the budget),
  // then kick chunk 1 in its own invocation. Rendering the first chunk here proves
  // the pipeline advances even if a later self-hop is flaky.
  const dead = new Set(job.deadModels || []);
  try {
    const { report, model } = await aiRouteChunk(buildChunkMessages(job.chunks[0], true, facts), env, dead);
    job.title   = report.title   || 'Report';
    job.summary = report.summary || '';
    job.meta    = report.meta    || null;
    job.blocks  = (job.blocks || []).concat(Array.isArray(report.blocks) ? report.blocks : []);
    ...
    console.error('[facts→chunk0] failed:', e.message);
    job.lastErr = e.message;
    if (e.attempts) job.lastAttempts = e.attempts;
  }
  job.deadModels = [...dead];
  ...
  if (job.total <= 1) {
    // only one chunk → finalize now
    await finalizeJob(job, env);
  } else {
    await kickStage(env, ctx, runId, 'chunk');   // render chunk 1+ in own invocations
  }
}

/* Finalize a job's accumulated blocks into the report doc. */
async function finalizeJob(job, env) {
  const generatedAt = Date.now();
  if (!job.blocks || job.blocks.length === 0) {
    await kvPut(env, 'td_report', { status: 'degraded', report: null,
      error: job.lastErr ? `AI failed: ${job.lastErr}` : 'All chunks failed to render.',
      generatedAt, model: null, runId: job.runId, lastAttempts: job.lastAttempts || null });
  } else {
    const report = { title: job.title || 'Report', summary: job.summary || '', meta: job.meta || undefined, blocks: job.blocks };
    await kvPut(env, 'td_report', { status: 'ready', report, error: null, generatedAt, model: job.model, runId: job.runId });
...
async function runStage(runId, env, ctx) {
  ...
  // Dead-model memory carried across chunks (persisted on the job in KV) so a
  // throttled model is skipped after its first failure instead of timing out
  // again on every remaining chunk.
  const dead = new Set(job.deadModels || []);

  try {
    // Inject live facts into every chunk so each section uses real numbers.
    const { report, model } = await aiRouteChunk(buildChunkMessages(chunk, isFirst, job.facts || ''), env, dead);
    if (isFirst) {
      job.title   = report.title   || 'Report';
      job.summary = report.summary || '';
      job.meta    = report.meta    || null;
    ...
    job.lastErr = e.message;
    if (e.attempts) job.lastAttempts = e.attempts;   // surfaced via /diag
...
/* Same router as aiRoute but parses chunk (blocks-only) responses.
   `dead` is a Set of "provider:model" keys already known-dead THIS build —
   shared across chunks so a throttled model (e.g. 2.5-pro) is tried once total,
   not re-discovered (and re-timed-out) on every chunk. Mutated in place. */
async function aiRouteChunk(messages, env, dead) {
  dead = dead || new Set();
  const attempts = [];
  const deadline = Date.now() + CHUNK_WALK_BUDGET_MS;   // hard stop for this walk
  for (const tier of AI_CHAIN) {
    for (const model of tier.models) {
      const key = `${tier.provider}:${model}`;
      if (dead.has(key)) continue;                       // skip known-dead this run
      // Bail before starting a model we don't have time to finish — better to
      // degrade with a reason than get the whole invocation killed mid-call.
      if (Date.now() > deadline) {
        const err = tagged('WALK_TIMEOUT', `chunk walk exceeded ${CHUNK_WALK_BUDGET_MS}ms`);
        err.attempts = attempts; throw err;
      }
      for (let retry = 0; retry <= MAX_RETRIES; retry++) {
        try {
          const txt = await dispatch(tier.provider, model, messages, env, CHUNK_MAX_OUT_TOKENS);
          const report = parseReport(txt, true);
          return { report, model: key, attempts };
        } catch (e) {
          attempts.push({ model: key, code: e.code || 'ERR' });
          if (e.code === 'NO_KEY' || e.code === 'NO_PROVIDER') { dead.add(key); break; }
          const isTimeout = e.name === 'TimeoutError';
          // Provider-level failure likely to persist for the rest of THIS build →
          // remember it so other chunks skip this model entirely.
          if (e.code === 'RATE_LIMIT' || e.code === 'SERVER' || isTimeout) dead.add(key);
          // Retry only transient non-timeout failures. A model that already timed
          // out won't un-timeout in 800ms — retrying just burns another 24s.
          const retryable = (e.code === 'RATE_LIMIT' || e.code === 'SERVER' || e.code === 'EMPTY') && !isTimeout;
          if (retryable && retry < MAX_RETRIES && Date.now() < deadline) { await sleep((e.retryAfter ? e.retryAfter*1000 : 800*Math.pow(2,retry)) + Math.random()*400); continue; }
          break;
        }
      }
    }
...
async function handle(request, env, ctx) {
  ...
  // ── health ──
  if (path === '/' && method === 'GET') {
    return json({ ok: true, service: 'trade-dashboard', version: VERSION, daily: DAILY_LIMIT, intervalSec: MIN_INTERVAL_S }, 200, request);
  }

  // ── diag: read-only snapshot of the last build so failures are visible.
  //    Hit https://trade-dashboard.av1.workers.dev/diag after a failed Run.
  //    `version` confirms the deploy landed; `report.status` + `job` show WHY:
  //      - report.status 'degraded' + lastAttempts full of code:'RATE_LIMIT' → APIs exhausted
  //      - lastAttempts full of code:'TimeoutError'                         → providers slow
  //      - job.stage stuck at 0 + factsDone:false                          → facts stage hung
  //    No secrets are exposed. ──
  if (path === '/diag' && method === 'GET') {
    const report = await kvGet(env, 'td_report');
    const job = await kvGet(env, 'td_job');
    const rs = await rateStatus(env).catch(() => null);
    return json({
      version: VERSION,
      now: Date.now(),
      rate: rs,
      report: report ? { status: report.status, error: report.error, model: report.model,
                         generatedAt: report.generatedAt, startedAt: report.startedAt,
                         elapsedSec: report.startedAt ? Math.round((Date.now() - report.startedAt) / 1000) : null,
                         lastAttempts: report.lastAttempts || null } : null,
      job: job ? { runId: job.runId, stage: job.stage, total: job.total, single: job.single,
                   factsDone: job.factsDone, factsLen: (job.facts || '').length,
                   deadModels: job.deadModels || [], lastErr: job.lastErr || null,
                   lastAttempts: job.lastAttempts || null } : null,
    }, 200, request);
  }
    ...
    if (r && r.status === 'building' && r.startedAt) {
      ...
      if (elapsed > limit) {
        const stale = { ...r, status: 'degraded', error: 'Build timed out. Try again — or shorten the prompt.',
                        diag: job ? { stage: job.stage, total: job.total, factsDone: job.factsDone,
                                      deadModels: job.deadModels || [], lastErr: job.lastErr || null,
                                      lastAttempts: job.lastAttempts || null } : null };
