 ...
 *            + Finnhub earnings + deterministic macro calendar + GDELT headlines
 ...
 *   GEMINI_KEY     Google AI Studio key (Gemini + grounding)
...
/* Watchlist — only used for the earnings facts (per-ticker analysis is the News
   tab's job; MacroBoard stays top-down). */
const WATCHLIST = ['NVDA','AMD','MU','INTC','WDC','SNDK','CRDO','MRVL','AVGO','GOOGL',
  'MSFT','AAPL','AMZN','META','TSLA','PLTR','CRWD','NET','SCCO','WMT','BABA','BE',
  'LMT','XOM','CVX','VLO','CNQ','MS','NVS','LMND','EXPE','ERO'];
...
/* GDELT narrative buckets — free, no key, 15-min fresh. artlist headlines + a
   volume read = "is this story actually hot right now".
   RULES: keep queries SIMPLE. GDELT chokes on long nested boolean. Max ~3 OR
   terms, short phrases. sourcelang:eng = English results only.
   Fall back to 48h timespan so thin-news days still return something. */
const GDELT_BUCKETS = [
  { key:'geopolitics', label:'Geopolitics / Middle East',
    q:'"Middle East" OR Iran OR Houthi OR Ukraine sourcelang:eng' },
  { key:'tariffs',     label:'Trade / Tariffs / Trump',
    q:'tariff OR "trade war" OR "Trump tariff" sourcelang:eng' },
  { key:'fed',         label:'Fed / Rates / Inflation',
    q:'"Federal Reserve" OR "interest rates" OR FOMC sourcelang:eng' },
  { key:'oil',         label:'Oil / OPEC / Energy',
    q:'OPEC OR "oil prices" OR "crude oil" sourcelang:eng' },
];
...
/* GDELT artlist + volume (article count = heat proxy). */
async function gdeltBucket(bucket){
  const base='https://api.gdeltproject.org/api/v2/doc/doc';
  // Try 24h first; fall back to 48h if empty (thin-news periods)
  for(const span of ['24h','48h']){
    const url=`${base}?query=${encodeURIComponent(bucket.q)}&mode=artlist&maxrecords=20&sort=datedesc&timespan=${span}&format=json`;
    try{
      const r=await fetch(url,{signal:AbortSignal.timeout(10000),headers:{'User-Agent':'Mozilla/5.0 (compatible; MacroBoard/1.0)','Accept':'application/json'}});
      if(!r.ok){ console.warn('[gdelt] HTTP',r.status,'for',bucket.key,span); continue; }
      const j=await r.json();
      const arts=(j.articles||[]).map(a=>({ title:a.title, domain:a.domain, seendate:a.seendate }));
      const seen=new Set(); const uniq=[];
      for(const a of arts){ const k=(a.title||'').toLowerCase().slice(0,80); if(k&&!seen.has(k)){ seen.add(k); uniq.push(a); } }
      if(uniq.length>0) return { key:bucket.key, label:bucket.label, count:uniq.length, articles:uniq.slice(0,10), span };
      // empty — try wider span
    }catch(e){ console.warn('[gdelt]',bucket.key,span,e.message); }
  }
  return { key:bucket.key, label:bucket.label, count:0, articles:[], span:'48h' };
}
async function gdeltAll(){
  return await Promise.all(GDELT_BUCKETS.map(gdeltBucket));
}
...
async function buildFacts(env){
  ...
  const [yq, fred, earnings, gdelt] = await Promise.all([
  ...
  // GDELT headline buckets + heat
  let news='LIVE NEWS HEADLINES (GDELT, last 24h — heat = article volume):\n';
  for(const b of gdelt){
    news+=`\n[${b.label}]  heat:${b.count}\n`;
    for(const a of b.articles.slice(0,6)) news+=`  - ${(a.title||'').slice(0,130)} (${a.domain})\n`;
  }
  ...
  return {
    text: [dateLine,'',market,'',macro,'',calText,'',ernText,'',news].join('\n'),
    gdeltHeat: gdelt.map(b=>({key:b.key,label:b.label,count:b.count})),
    haveData: { yahoo:Object.keys(yq).length, fred:Object.keys(fred).length, earnings:ern.length },
  };
}
...
async function callGemini(model, sys, user, env, maxTokens, grounded){
  ...
  const r=await fetchTO(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
...
async function callOAICompat(baseUrl, key, model, sys, user, maxTokens){
  if(!key) throw tagged('NO_KEY','key not set');
  const r=await fetchTO(`${baseUrl}/chat/completions`,{
...
const callNvidia=(m,sys,user,env,mt)=>callOAICompat('https://integrate.api.nvidia.com/v1',env.NVIDIA_KEY,m,sys,user,mt);
const callGroq  =(m,sys,user,env,mt)=>callOAICompat('https://api.groq.com/openai/v1',env.GROQ_KEY,m,sys,user,mt);
...
const STAGE_A_SYS = `You are a senior macro strategist for an active LONG-ONLY equity trader. Using Google Search for anything time-sensitive, produce TODAY'S market-moving macro picture. Verify the provided headlines against live search; correct anything stale. Be specific, current, and cite real developments. Return ONE JSON object ONLY (no markdown, no prose outside it):
{
...
async function runStageA(facts, env){
  const user = `${facts.text}\n\nProduce the macro JSON for an active long-only trader. Use live Google Search to confirm the very latest on geopolitics, Trump/policy, the Fed, and oil. Correct any stale headline above.`;
  const attempts=[];
  for(const model of GROUNDED_MODELS){
    try{
      const txt=await callGemini(model, STAGE_A_SYS, user, env, STAGE_A_TOKENS, true);
      const obj=parseObj(txt);
      return { narrative:obj, model:`gemini:${model} (grounded)`, attempts };
    }catch(e){ attempts.push({model:`gemini:${model}`,code:e.code||'ERR',msg:(e.message||'').slice(0,100)}); }
  }
  // grounding failed entirely → fall through with no narrative; Stage B still
  // renders from hard data (degraded but not broken).
...
6. scoreboard — "Sector Strength": strongest sectors (good) and sectors to avoid (bad) for longs today.
...
async function runStageB(facts, narrative, env){
  ...
  for(const tier of RENDER_CHAIN){
    for(const model of tier.models){
      try{
        let txt;
        if(tier.provider==='gemini') txt=await callGemini(model,sys,user,env,STAGE_B_TOKENS,false);
        else if(tier.provider==='nvidia') txt=await callNvidia(model,sys,user,env,STAGE_B_TOKENS);
        else txt=await callGroq(model,sys,user,env,STAGE_B_TOKENS);
        const obj=parseObj(txt);
      ...
      }catch(e){ attempts.push({model:`${tier.provider}:${model}`,code:e.code||'ERR',msg:(e.message||'').slice(0,100)}); }
    }
  }
...
async function stageNarrative(runId, env){
  ...
  if(job.facts){
    ...
    catch(e){ console.error('[narrative]',e.message); job.deadStageA=true; job.lastAttempts=e.attempts||null; }
  }
  job.phase='render';
  await kvPut(env,'td_job',job);
...
async function stageRender(runId, env){
  ...
  try{
    const {report,model}=await runStageB(job.facts||{text:''}, job.narrative, env);
...
async function handle(request, env, ctx){
  ...
  if(path==='/diag'&&method==='GET'){
    ...
    return json({ version:VERSION, now:Date.now(), rate:rs,
      ...
      job: job?{runId:job.runId,phase:job.phase,cron:job.cron,deadStageA:job.deadStageA,haveFacts:!!job.facts,haveNarrative:!!job.narrative,narrativeModel:job.narrativeModel,gdeltHeat:job.facts?.gdeltHeat||null,haveData:job.facts?.haveData||null,lastAttempts:job.lastAttempts||null}:null,
    },200,request);
  }
  ...
  if(path==='/debug-data'&&method==='GET'){
    ...
    return json({ ok:true, haveData:facts.haveData, gdeltHeat:facts.gdeltHeat, factsLength:facts.text.length, factsPreview:facts.text.slice(0,1500) },200,request);
  }
...
/* ════════════════════════ CRON ════════════════════════
   3× premarket/intraday Mon-Fri pre-warm → build + cache so the user just opens
