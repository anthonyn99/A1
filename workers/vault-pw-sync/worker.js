// ─────────────────────────────────────────────────────────────────────────────
// vault-pw-sync — Firestore proxy for the Vault extension's PASSWORD manager.
//
// WHY THIS EXISTS
...
//
//     dashboards/vault_pw  →  { config, items:{ id -> encDoc }, savedAt }
//
// ENDPOINTS  (all require header  X-Vault-Key: <VAULT_KEY secret>)
//   GET  /vault   → { config, items, savedAt }        (encrypted)
//   PUT  /vault   ← { config, items }                 (encrypted; savedAt stamped)
//
// SECRETS: FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY, VAULT_KEY
// ─────────────────────────────────────────────────────────────────────────────

const DOC_PATH = "dashboards/vault_pw";
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== "/vault" && url.pathname !== "/") {
