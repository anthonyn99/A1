// ─────────────────────────────────────────────────────────────────────────────
// warden-pw-sync — Firestore proxy for the Warden Launcher's PASSWORD manager.
//
// WHY THIS EXISTS
// Same reason as keychain-sync: the Firebase project enforces App Check
// (reCAPTCHA v3), which a chrome-extension:// page cannot satisfy. So the
// extension reads the encrypted password vault through this Worker, which
// authenticates with the Firebase service account (bypassing App Check + rules).
//
// ── ZERO-KNOWLEDGE ──────────────────────────────────────────────────────────
// The document served here is END-TO-END ENCRYPTED (see vault-crypto.js): it
// contains ONLY salts, wrapped keys, and AES-256-GCM ciphertext. This Worker —
// and the shared key that gates it — can NEVER decrypt anything. Only a client
// holding the user's master password (or recovery key / biometric) can. So even
// though the proxy is protected by a shared key, that key alone is useless: it
// grants access to ciphertext, not plaintext.
//
//     dashboards/warden_pw  →  { config, items:{ id -> encDoc }, savedAt }
//
// ENDPOINTS  (all require header  X-Warden-Key: <WARDEN_KEY secret>)
//   GET  /warden   → { config, items, savedAt }        (encrypted)
//   PUT  /warden   ← { config, items }                 (encrypted; savedAt stamped)
//
// SECRETS: FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY, WARDEN_KEY
// ─────────────────────────────────────────────────────────────────────────────

const DOC_PATH = "dashboards/warden_pw";
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== "/warden" && url.pathname !== "/") {
      return json({ error: "not found" }, 404);
    }

    if (!env.WARDEN_KEY || request.headers.get("X-Warden-Key") !== env.WARDEN_KEY) {
    ...
    try {
      ...
      if (request.method === "GET") {
        ...
        if (r.status === 404) return json({ config: null, items: {}, savedAt: 0 });
        if (!r.ok) return json({ error: "firestore read " + r.status, detail: await r.text() }, 502);
        const doc = await r.json();
        const f = doc.fields || {};
        return json({
          config: f.config ? decode(f.config) : null,
          items: f.items ? decode(f.items) : {},
      ...
      if (request.method === "PUT") {
        ...
        const payload = {
          fields: {
            config: encode(sanitize(body.config || null)),
            items: encode(sanitize(body.items || {})),
