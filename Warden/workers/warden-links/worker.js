// ─────────────────────────────────────────────────────────────────────────────
// warden-links — Firestore proxy for the Warden Launcher extension.
//
// WHY THIS EXISTS
// Her Firebase project enforces App Check (reCAPTCHA v3), which blocks direct
// Firestore access from any origin that cannot mint a reCAPTCHA token — and a
// chrome-extension:// page never can, even holding a valid anonymous ID token.
//
// So the extension talks to this Worker instead. It authenticates to Firestore
// with her Firebase SERVICE ACCOUNT, which bypasses both App Check and security
// rules, and gates itself behind a shared header key. It reads and writes one
// document:
//
//     dashboards/warden_links  →  { connections, colmap, savedAt }
//
// the same document warden.html's Links tab uses, so the PWA and the extension
// stay in sync both directions.
//
// ENDPOINTS  (both require header  X-Warden-Key: <WARDEN_KEY secret>)
//   GET  /links   → { connections, colmap, savedAt }
//   PUT  /links   ← { connections, colmap }            (savedAt set server-side)
//   "/" is accepted as an alias for /links.
//
// AUTH
//   A single key, WARDEN_KEY. The original this was ported from carried a
//   two-key split because one Worker served both Tony's Keychain and Veda's
//   Links; this one serves only her Links, so that complexity is gone.
//
//   The key is compiled into a distributable extension, so treat it as a gate
//   that stops casual enumeration and overwriting — not as a secret. It must
//   NEVER be the key Tony's Vault Launcher ships.
//
// SECRETS (wrangler secret put …): FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL,
//   FIREBASE_PRIVATE_KEY, WARDEN_KEY
// ─────────────────────────────────────────────────────────────────────────────

// Route → Firestore document. "/" is an alias so a bare-origin request works.
const DOC_PATHS = {
  "/links": "dashboards/warden_links",
  "/": "dashboards/warden_links"
};

const CORS = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "GET, PUT, OPTIONS",
  "Access-Control-Allow-Headers": "Content-Type, X-Warden-Key",
...
export default {
  async fetch(request, env) {
    ...
    // One document, one key. The two-key split the original needed (Tony's
    // Keychain and Veda's Links behind the same Worker) does not apply here:
    // this Worker serves only her Links.
    const expected = env.WARDEN_KEY;
    if (!expected || request.headers.get("X-Warden-Key") !== expected) {
