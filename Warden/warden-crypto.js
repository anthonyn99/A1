/* ─────────────────────────────────────────────────────────────────────────────
 * warden-crypto.js — Warden Password Manager · cryptographic core
 *
 ...
 *           key — the warden itself is never re-encrypted.
...
(function (global) {
  'use strict';
  if (global.WardenCrypto) return;

  const subtle = (global.crypto && global.crypto.subtle) || null;
  const getRandomValues = (arr) => global.crypto.getRandomValues(arr);

  // ── Versioned parameters ──────────────────────────────────────────────────
  // `kdf.v` is stored with every warden so we can raise iterations or migrate to
  // Argon2id later WITHOUT breaking existing wardens: unwrap with the stored
  ...
  const VERIFIER_PLAINTEXT = 'warden.verify.v1';
  ...
  // Generate the warden's random Data Encryption Key. Extractable so it can be
  ...
  async function unwrapDEK(wrapObj, kek) {
    ...
    return subtle.unwrapKey(
      ...
      // via subtle.wrapKey. This mirrors how Bitwarden/1Password keep the warden
  ...
  // ── High-level warden lifecycle ────────────────────────────────────────────

  // Create a brand-new warden. Returns the cloud-safe `config`, the live in-
  ...
  async function createWarden(masterPassword, opts) {
    opts = opts || {};
  ...
  // Requires a live DEK (i.e. the warden is already unlocked). Only re-wraps the
  ...
  // whoever holds the key can already read the warden. Throws 'bad-recovery' on a
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = global.WardenCrypto;
