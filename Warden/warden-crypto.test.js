// Real verification of warden-crypto.js against Node's WebCrypto.
...
(async () => {
  console.log('\n── createWarden + password unlock ──');
  const { config, dek, recoveryCode } = await VC.createWarden('Correct-Horse-Battery-Staple-9!');
  ...
  ok('new password unlocks the SAME warden (item still decrypts)', JSON.stringify(await VC.decrypt(dekN, blob)) === JSON.stringify(item));
  ...
  ok('reset password unlocks the SAME warden (item still decrypts)', JSON.stringify(await VC.decrypt(dekReset, blob)) === JSON.stringify(item));
  ...
  const hinted = await VC.createWarden('Hinted-Master-PW-4!', { hint: 'the usual + birth year' });
  ok('hint stored on the config', hinted.config.hint === 'the usual + birth year');
  ok('warden created without opts has an empty hint', config.hint === '');
  ...
  ok('new warden has a securityStamp', !!config.securityStamp);
  ...
  const a = await VC.createWarden('same-pw'); const b = await VC.createWarden('same-pw');
  ok('two wardens with same password have different salts', a.config.master.salt !== b.config.master.salt);
