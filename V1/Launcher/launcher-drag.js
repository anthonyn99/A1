// Card drag-to-reorder for the Launcher popup.
//
// Deliberately the same mechanic as the Links program's card DnD (index.html,
// vdkc_onCardDown / vdkc_onCardMove / vdkc_onCardUp): a dashed placeholder takes
// the card's slot in the flow, a `position:fixed` clone follows the pointer, and
// on drop the placeholder position decides the new order. Pointer events cover
// mouse, pen and touch with one code path, so this works on a phone.
//
...
// or half the columns are unreachable.
//
// The caller owns persistence — LauncherDrag reports the new order and the
...
const LauncherDrag = (() => {
  ...
  function enable(listEl, opts) {
    ...
    function onUp() {
      ...
      // exactly the shape dashboards/veda_links stores (colmap[connIdx] = col).
      const dom = cards();
      ...
      bind();                                  // the moved card's grip lost nothing, but rebind is cheap
...
if (typeof globalThis !== "undefined") globalThis.LauncherDrag = LauncherDrag;
