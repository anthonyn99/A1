...
## 0a. INSTALL STATUS — V1, 2026-08-04

Installed into V1 by Claude. What differs from this README's assumptions, and
what is left.

**V1 is not a plain static site.** It is a build-pipeline suite: `scripts/build.mjs`
generates `dist/`, and `workers/site` serves only that. So §2's "copy the folder
and open `StudyOS/studyos.html`" does not apply. The source files live at the V1
root (`studyos.html`, `css/`, `js/`, `config/`, `assets/`) and the build copies
them to `dist/studyos/`, plus `firebase-messaging-sw.js` to the site ROOT.

| Step | State |
|---|---|
| §2 move into V1 | **done** — via `build.mjs`, served at `/studyos/` |
| §2 root service worker | **done** — `build.mjs` `rootFiles` copies it to `dist/` |
| §2 `shell.nav` | **done** — TradeBoard `/`, Finance `/finance/` |
| §3 Firebase config | untouched, as required — still the Index project |
| §3 App Check allowlist | ⚠️ **MANUAL — REQUIRED, NOT DONE** (see below) |
| §4 studyos-files Worker | **deployed** → `https://studyos-files.vedapatel05.workers.dev` |
| §5 studyos-api Worker | **deployed** → `https://studyos-api.vedapatel05.workers.dev` |
| §6 Formspree | ⚠️ **MANUAL — NOT DONE** (App Lock recovery only) |
| §7 push secrets | ⚠️ **MANUAL — NOT DONE** (`/health` reports `fcm:false`) |
| §8 App Lock password | manual, in-app, whenever Veda wants it |

KV namespaces created on Veda's Cloudflare account (`vedapatel05`):
`FILES` = `f3a844dd958e483eaaf0c2faa763efc2`,
`TOKEN_CACHE` = `25cb36ce33e54ac3959447f556cf3b22`.

Verified: `/usage` → `{"bytes":0,"files":0}`; api `/health` → `{"ok":true,"kv":true,"fcm":false}`.

### ⚠️ `firebase/firestore.rules` is V1's OWN file — do not confuse it with §3

§3 says "there is deliberately no `.rules` file in this project". In V1 there
now is one, but it is **not StudyOS's and not Index's**. It predates this
migration (commits `2698906`, `e62216d`) and governs a *different* Firebase
project: `firebase/.firebaserc` → `tradeboard-6b2ea`, covering TradeBoard and
Finance only.

StudyOS talks to Index (`task-dashboard-d2b53`). The two projects never
interact, so this file is harmless to StudyOS and StudyOS is harmless to it.
§3's "never deploy Firestore rules" means **never deploy rules to the Index
project** — `firebase.json` here deploys only to `tradeboard-6b2ea`, so
`npx firebase-tools deploy --only firestore:rules` remains safe for V1 and
still must never be pointed at Index.

### CI note

`workers/studyos-files` and `workers/studyos-api` had a `package.json` with no
pinned wrangler, which the deploy workflow's own comments call out as the cause
of a past opaque failure (`npx --no-install wrangler` falling back to the
runner's cached v3). Both now declare `wrangler ^4.0.0`, matching `workers/site`.
