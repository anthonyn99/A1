/* ─────────────────────────────────────────────────────────────────────────────
 * vault-pay-ui.js — Vault · Payments tab (PWA)
 *
 * The Google-Wallet-flavoured half of Vault: saved cards shown as card faces,
 * masked by default, revealed only after an identity check. Passwords stay
 * Google-Password-Manager-flavoured next door; both are the same vault.
 *
 * ── How it plugs in ─────────────────────────────────────────────────────────
 ...
 *    or-master-password check the vault already uses for rotating a recovery
 ...
 *
 * Depends on: vault-pay.js (payment logic), vault-ui.js (host + shell).
 * ──────────────────────────────────────────────────────────────────────────── */

(function () {
  'use strict';
  if (window.VaultPayUI) return;

  var PAY = window.VaultPay;
  var KIND = 'payment';
  ...
  // Locking the vault must also close the grace window — otherwise a re-unlock
  ...
  function cardRow(it, host, searching) {
    ...
    var head = el('div', { class: 'vault-row vpay-head' }, [
      ...
      el('div', { class: 'vault-row-main' }, [
        ...
        el('div', { class: 'vault-row-sub' }, [s.subtitle + (s.expiry ? ' · ' + s.expiry : '')]),
      ]),
      chip,
      star,
    ...
    var cvvVal = el('span', { class: 'vault-acc-pw' }, [it.cvv ? '•••' : '—']);
    var revBtn = el('button', {
      class: 'vault-icon', title: 'Reveal card details', html: host.icons.eye,
    ...
    var main = el('div', { class: 'vault-acc-main' }, [
      ...
      el('div', { class: 'vault-acc-line' }, [
        el('div', { class: 'vault-acc-field' }, [
          el('span', { class: 'vault-acc-flabel' }, ['Security code']), cvvVal,
        ]),
      ]),
    ]);
    ...
    return el('div', {
      class: 'vault-site vpay-site' + (s.expiryState === 'expired' ? ' vpay-expired' : ''),
      'data-id': it.id,
    }, [head, body]);
  }
  ...
  // across because VaultPay.normalize() only shapes the BODY.
  ...
  function openEditor(item, host) {
    ...
    function field(label, value, opts) {
      ...
      var input = el(opts.textarea ? 'textarea' : 'input', {
        class: 'vault-input', type: opts.type || 'text', value: value == null ? '' : value,
        placeholder: opts.ph || '', autocomplete: 'off', inputmode: opts.inputmode || null, maxlength: opts.max || null,
      });
      return { input: input, node: el('div', { class: 'vault-field' }, [el('label', { class: 'vault-flabel' }, [label]), input]) };
    }
    function selectField(label, options, selected) {
      ...
      return { input: sel, node: el('div', { class: 'vault-field' }, [el('label', { class: 'vault-flabel' }, [label]), sel]) };
    }
    ...
    var numInput = el('input', { class: 'vault-input', type: 'password', value: PAY.formatNumber(item.number) || '', placeholder: '1234 5678 9012 3456', autocomplete: 'off', inputmode: 'numeric' });
    var netBadge = el('span', { class: 'vpay-netbadge', html: PAY.brandMark(PAY.networkOf(item).id) });
    var numShown = false;
    var numRev = el('button', {
      class: 'vault-icon', type: 'button', title: 'Show', html: host.icons.eye,
      onclick: async function () {
    ...
    var cvvInput = el('input', { class: 'vault-input', type: 'password', value: item.cvv || '', placeholder: '•••', autocomplete: 'off', inputmode: 'numeric', maxlength: String(PAY.cvvLength(item)) });
    var cvvShown = false;
    ...
    var saveBtn = el('button', { class: 'vault-btn primary' }, [isNew ? 'Add card' : 'Save']);
    ...
    var actions = [saveBtn, el('button', { class: 'vault-btn', onclick: close }, ['Cancel'])];
    if (!isNew) {
      actions.push(el('button', {
        class: 'vault-btn danger', onclick: async function () {
    ...
    var box = el('div', { class: 'vault-modal', onclick: function (e) { e.stopPropagation(); } }, [
      el('div', { class: 'vault-modal-title' }, [isNew ? 'Add payment method' : 'Edit payment method']),
      nickname.node, type.node, holder.node,
      el('div', { class: 'vault-field' }, [
        el('label', { class: 'vault-flabel' }, ['Card number']),
        el('div', { class: 'vault-pw-input' }, [numInput, netBadge, numRev]),
      ]),
      el('div', { class: 'vpay-grid3' }, [expMonth.node, expYear.node,
        el('div', { class: 'vault-field' }, [
          el('label', { class: 'vault-flabel' }, ['Security code']),
          el('div', { class: 'vault-pw-input' }, [cvvInput, cvvRev]),
        ]),
      ]),
      el('label', { class: 'vault-ack' }, [favCb, el('span', {}, ['Pin this card to the top'])]),
      ...
      el('div', { class: 'vault-field' }, [
        ...
        el('button', { class: 'vault-btn', type: 'button', style: 'width:auto;margin:2px 0 0;padding:8px 12px', onclick: function () { customFields.push({ label: '', value: '' }); renderCF(); } }, ['+ Add custom field']),
      ]),
      err, warn,
  ...
  function injectStyles() {
    if (document.getElementById('vault-pay-styles')) return;
    var css = [
      ...
      '.vpay-chip.warn{background:transparent;border:1px solid rgba(205,174,116,.36);color:#e8d5ab}.vpay-chip.bad{background:transparent;border:1px solid rgba(214,138,124,.45);color:#d68a7c}',
      ...
      '.vpay-face-star{color:#eddcb8;line-height:0;display:inline-flex}.vpay-face-star svg{width:14px;height:14px;display:block}',
      ...
      '.vpay-warn{color:#e8d5ab;font-size:11.5px;min-height:14px;margin:-4px 0 4px;text-align:left;line-height:1.5}',
      // mobile
  ...
  window.VaultPayUI = { render: render, fillList: fillList, openEditor: openEditor, resetGrace: resetGrace, KIND: KIND };
})();
