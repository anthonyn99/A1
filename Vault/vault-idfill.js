// ─────────────────────────────────────────────────────────────────────────────
// vault-idfill.js — Vault extension · identity-document field detection + filling
//
// The ID Docs twin of vault-cardfill.js. Runs in the extension's ISOLATED world
...
//                     to offer the "Vault ID Docs" dropdown and where to anchor.
...
(function () {
  ...
  // ── haystacks (same two normalisations vault-cardfill.js uses) ─────────────
  ...
  // subtle case: vault-cardfill.js classifies a bare "State" box as
  ...
  function claimedByCard(elm) {
    try {
      if (!self.VaultCardFill) return false;
      var t = self.VaultCardFill.classify(elm);
      if (!t) return false;
  ...
  // half-filled. vault-cardfill.js groups by form for the same reason.
  //
  ...
  function probe(root) {
    try { return !!(self.VaultCardFill && self.VaultCardFill.hasCardFields(root || undefined)); }
    catch (e) { return false; }
  }
  ...
  // Fill this frame from the bundle produced by VaultId.autofillValues().
