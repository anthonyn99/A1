// ─────────────────────────────────────────────────────────────────────────────
// vault-id-panel.js — Vault extension · ID Docs panel UI (popup)
//
// The identity-document twin of vault-pay-panel.js. Same data layer
// (vault-pw-core.js), same unlocked session, same 30-minute idle window —
// unlocking on any tab unlocks this one too, because there is only ONE vault.
//
...
// FRESH credential check (VaultPWCore.authFresh()): a master password or
...
(function () {
  ...
  async function render() {
    ...
    if (!panel) return;
    if (typeof VaultCrypto === "undefined" || !VP || !VID) {
  ...
  // ── unlock (identical flow to the other tabs — one vault, one session) ─────
  ...
  async function renderList() {
    ...
    function draw(q) {
      ...
      if (!list.length) {
        listWrap.appendChild(el("div", { class: "pw-msg" }, [q ? "No matches." : "No ID documents yet. Add one in the Vault app → ID Docs."]));
        return;
      }
  ...
  function row(d) {
    ...
    revBtn = el("button", {
      class: "pw-icon", title: "Reveal document number", html: (window.VaultIcons || {}).eye,
      onclick: async () => {
        VP.touchSession();
    ...
    if (s.number) {
      actions.push(el("button", {
        class: "pw-icon", title: "Copy document number", html: (window.VaultIcons || {}).copy,
        onclick: async () => {
    ...
    if (s.attachments) {
      actions.push(el("button", {
        ...
        html: (window.VaultIcons || {}).upload || paperclipIcon(),
  ...
  async function fillActiveTab(d) {
    ...
    withActiveTab((tabId) => {
      chrome.runtime.sendMessage({ action: "vaultFillIdDoc", id: d.id, tabId }, (r) => {
  ...
  function attachToActiveTab(d) {
    ...
    withActiveTab((tabId) => {
      toast("Attaching…");
      chrome.runtime.sendMessage({ action: "vaultAttachIdFile", id: d.id, tabId }, (r) => {
        void chrome.runtime.lastError;
        if (!r || !r.ok) {
  ...
  async function saveScan(d, btn) {
    ...
    try {
      ...
      toast(e && e.message === "locked" ? "Vault is locked" : "Couldn't download that scan");
  ...
  window.VaultIdPanel = { render, lock: () => VP.lock() };
})();
