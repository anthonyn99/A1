// ─────────────────────────────────────────────────────────────────────────────
// vault-pay.js panel — Vault extension · Payments panel UI (popup)
//
// The payments twin of vault-pw.js. Same data layer (vault-pw-core.js), same
// unlocked session, same 30-minute idle window — unlocking on the Passwords tab
// unlocks this one too, because there is only ONE vault.
//
// ── What's stricter here than for passwords ─────────────────────────────────
// Cards are masked by default. Revealing a full number or a CVV — and copying
// either — requires a FRESH credential check (VaultPWCore.authFresh()): a
...
(function () {
  ...
  async function render() {
    ...
    if (!panel) return;
    if (typeof VaultCrypto === "undefined" || !VP || !PAY) { clear(); panel.appendChild(el("div", { class: "pw-msg" }, ["Payments module not loaded."])); return; }
    let has;
  ...
  // ── unlock (identical flow to the Passwords tab — one vault, one session) ──
  ...
  window.VaultPayPanel = { render, lock: () => VP.lock() };
})();
