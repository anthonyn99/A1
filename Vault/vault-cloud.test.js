 ...
 *
 * vault-cloud.js is a browser IIFE that hangs itself off `window`, so it runs
...
function loadCloud() {
  ...
  vm.runInContext(fs.readFileSync(path.join(__dirname, 'vault-cloud.js'), 'utf8'), ctx, { filename: 'vault-cloud.js' });
  return ctx.VaultCloud;
}
