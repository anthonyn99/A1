/* ============================================================================
 * vault-cloud-ui.js — the Cloud tab
 *
 * Renders Vault's Cloud section. Every cloud call goes through VaultCloud (see
 * vault-cloud.js); this file holds a provider *id* and never knows whether it is
 ...
 * all in-page — Vault never uses alert/confirm/prompt.
 * ========================================================================== */
(function () {
  'use strict';

  var VC = null;                       // VaultCloud, resolved on first render
  function vc() { return VC || (VC = window.VaultCloud); }
  ...
  /* ── Styles ───────────────────────────────────────────────────────────────
   * Everything is scoped under #vault-cloud-panel and uses only Vault's own
  ...
  var STYLE_ID = 'vault-cloud-styles';
  function injectStyles() {
    ...
    s.textContent = [
      ...
      /* modals — Vault never uses browser dialogs */
  ...
  // Replaces window.confirm — same job, Vault's chrome.
  ...
  function buildStores() {
    ...
    vc().connected().forEach(function (p) {
      ...
      if (mine.length) {
        ...
        var colors = { image: '#8fb3d9', video: '#c49bd4', doc: '#e0b874', other: '#7d7d86' };
  ...
  function setupEmpty() {
    return el('div', { class: 'vcl-empty', style: 'padding:56px 20px' }, [
      ...
      el('div', { style: 'max-width:440px;margin:0 auto 18px', text: 'Vault talks to Google Drive and Dropbox directly from this page. You’ll need a client ID from each — it takes a minute and only has to be done once.' }),
