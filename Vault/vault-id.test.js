// Real verification of vault-id.js (pure logic) PLUS an end-to-end check that
// ID-document items ride the EXISTING vault-crypto/vault-store pipeline with
...
//
//   node vault-id.test.js
const VID = require('./vault-id.js');
...
(async () => {
  ...
  ok('an oversized thumbnail is dropped rather than bloating the vault doc',
  ...
  console.log('\n── encrypted at rest (real vault-crypto + vault-store) ──');
  const backend = VaultStore.memoryBackend();
  const { config, dek } = await VC.createVault('correct horse battery staple');
  ...
  const session = new VaultSession({ backend, deviceStore: VaultSession.memoryDeviceStore(), autoLockMs: 0 });
