/* ─────────────────────────────────────────────────────────────────────────────
 * vault-pay.js — Vault · payment-methods core (PURE logic, no DOM, no network)
 *
 * The single source of truth for everything payment-shaped, shared verbatim by
 * the PWA (vault-pay-ui.js) and the browser extension (popup panel, background
 * service worker, autofill content script) — exactly like vault-crypto.js is
 ...
 * vault-store.js item with `kind: 'payment'`, so it inherits the ENTIRE existing
 ...
 * and last-4 ALL live inside `enc` — AES-256-GCM under the vault's DEK. The
 * cloud never sees a single digit. See vault-crypto.js / vault-store.js.
 *
 * ── Extensibility ───────────────────────────────────────────────────────────
...
(function (global) {
  ...
  // ── Normalisation ─────────────────────────────────────────────────────────
  // Canonical shape for a payment BODY (the object vault-store.js encrypts).
  ...
  // caller has proven a fresh authentication (see vault-pw-core.authFresh()).
  ...
  var IMPORTERS = [
    {
      id: 'vault-json', label: 'Vault (plain JSON export)', format: 'json',
      note: 'A Vault payments export made from Settings → Import / Export.',
      detect: function (j) { return !!(j && j.format === 'vault-payments' && Array.isArray(j.items)); },
