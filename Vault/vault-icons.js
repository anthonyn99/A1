/**
 * vault-icons.js — shared line-icon set for the Vault UI.
 *
 ...
 * `.vault-icon svg` / `.pw-icon svg` rules), which keeps icon scale consistent
 * across the app instead of being re-declared per call site.
 *
 * Replaces the emoji glyphs the UI used to render. Loaded before vault-ui.js
 * (web app) and vault-pw.js / vault-pay-panel.js (extension popup).
 */
(function () {
  'use strict';
  if (window.VaultIcons) return;
  ...
  window.VaultIcons.starOn = window.VaultIcons.star.replace('fill="none"', 'fill="currentColor"');
})();
