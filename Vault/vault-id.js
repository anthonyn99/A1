/* ─────────────────────────────────────────────────────────────────────────────
 * vault-id.js — Vault · ID Documents core (PURE logic, no DOM, no network)
 *
 * The single source of truth for everything identity-document-shaped, in the
 * same spirit as vault-pay.js is for cards. Runs in the PWA and in Node ≥ 16.
 *
 * ── What it does NOT do ─────────────────────────────────────────────────────
 * No encryption, no storage, no DOM, no network. An ID document is just another
 * vault-store.js item with `kind: 'iddoc'`, so it inherits the ENTIRE existing
 ...
 * reference live inside `enc` — AES-256-GCM under the vault's DEK. The bytes of
 * an uploaded image/PDF are encrypted with the SAME DEK before they leave the
 * device (see vault-id-files.js); the file host only ever holds ciphertext.
 *
 * ── Extensibility ───────────────────────────────────────────────────────────
...
(function (global) {
  'use strict';
  if (global.VaultId) return;

  var KIND = 'iddoc';
  var EXPIRING_DAYS = 60;          // "Expires Soon" window
  var MAX_INLINE_THUMB = 9000;     // bytes — keeps the single vault doc small
